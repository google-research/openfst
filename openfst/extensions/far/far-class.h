...
#ifndef OPENFST_EXTENSIONS_FAR_FAR_CLASS_H_
#define OPENFST_EXTENSIONS_FAR_FAR_CLASS_H_
...
class FarReaderClassImpl : public FarReaderImplBase {
 public:
  explicit FarReaderClassImpl(absl::string_view source)
      : reader_(FarReader<Arc>::Open(source)) {}

  explicit FarReaderClassImpl(absl::Span<const std::string> sources)
...
using OpenFarReaderClassArgs = WithReturnValue<std::unique_ptr<FarReaderClass>,
 ...
 public:
  ...
  const FarReader<Arc>* GetFarReader() const {
    ...
    const FarReaderClassImpl<Arc>* typed_impl =
        absl::down_cast<FarReaderClassImpl<Arc>*>(impl_.get());
  ...
  static absl_nullable std::unique_ptr<FarReaderClass> Open(
...
void OpenFarReaderClass(OpenFarReaderClassArgs* args) {
  ...
  if (impl->GetFarReader() == nullptr) {
    ...
    args->retval = absl::WrapUnique(new FarReaderClass(std::move(impl)));
  }
}
...
class FarWriterClassImpl : public FarWriterImplBase {
 public:
  explicit FarWriterClassImpl(absl::string_view source,
 ...
 public:
  ...
  const FarWriter<Arc>* GetFarWriter() const {
    ...
    const FarWriterClassImpl<Arc>* typed_impl =
        absl::down_cast<FarWriterClassImpl<Arc>*>(impl_.get());
...
#endif  // OPENFST_EXTENSIONS_FAR_FAR_CLASS_H_
