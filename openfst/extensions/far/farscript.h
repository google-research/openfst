...
#ifndef OPENFST_EXTENSIONS_FAR_FARSCRIPT_H_
#define OPENFST_EXTENSIONS_FAR_FARSCRIPT_H_
...
#include "openfst/extensions/far/compile-strings.h"
#include "openfst/extensions/far/convert.h"
#include "openfst/extensions/far/create.h"
#include "openfst/extensions/far/encode.h"
#include "openfst/extensions/far/equal.h"
#include "openfst/extensions/far/extract.h"
...
#include "openfst/extensions/far/isomorphic.h"
#include "openfst/extensions/far/print-strings.h"
...
void CompileStrings(const std::vector<std::string>& sources,
                    FarWriterClass& writer, absl::string_view fst_type,
...
using FarConvertArgs =
    std::tuple<FarReaderClass&, FarWriterClass&, absl::string_view>;
...
bool Equal(FarReaderClass& reader1, FarReaderClass& reader2,
           float delta = kDelta, absl::string_view begin_key = "",
           absl::string_view end_key = "");

using FarExtractArgs =
    std::tuple<FarReaderClass&, int32_t, const std::string&, const std::string&,
...
void Info(FarInfoArgs* args) {
  ::fst::Info<Arc>(std::get<0>(*args), std::get<1>(*args), std::get<2>(*args),
                   std::get<3>(*args));
}
...
using FarIsomorphicInnerArgs =
    std::tuple<FarReaderClass&, FarReaderClass&, float, absl::string_view,
               absl::string_view>;
...
void Isomorphic(FarIsomorphicArgs* args) {
  ...
  FarReader<Arc>& reader2 = *std::get<1>(args->args).GetFarReader<Arc>();
  args->retval =
      ::fst::Isomorphic<Arc>(reader1, reader2, std::get<2>(args->args),
                             std::get<3>(args->args), std::get<4>(args->args));
}
...
#endif  // OPENFST_EXTENSIONS_FAR_FARSCRIPT_H_
