...
#ifndef OPENFST_EXTENSIONS_PDT_SHORTEST_PATH_H_
#define OPENFST_EXTENSIONS_PDT_SHORTEST_PATH_H_
...
class PdtShortestPathData {
 public:
  ...
  // TODO: Currently copying SearchState and passing a const reference to
 ...
 private:
  ...
  using SearchMap =
      absl::flat_hash_map<SearchState, SearchData, SearchStateHash>;

  using SearchMultimap = std::unordered_multimap<StateId, StateId>;

  // Hash map from paren spec to open paren data.
  using ParenMap = absl::flat_hash_map<ParenSpec, SearchData, ParenHash>;
...
void PdtShortestPathData<Arc>::GC(StateId start) {
  ...
  const auto [range_begin, range_end] = search_multimap_.equal_range(start);
...
class PdtShortestPath {
 public:
  ...
  PdtShortestPath(const Fst<Arc>& ifst,
      ...
      : ifst_(ifst.Copy()),
        ...
        error_(false) {
    // TODO: Make this a compile-time static_assert once:
 ...
 private:
  ...
  SpData sp_data_;
  absl::flat_hash_map<Label, Label> paren_map_;
...
inline void PdtShortestPath<Arc, Queue>::ProcOpenParen(Label paren_id,
  ...
  if (pdist != Plus(pdist, weight)) {
    ...
    for (auto set_iter = balance_data_.Find(paren_id, nextstate);
      ...
      const auto [cp_begin, cp_end] =
          close_paren_multimap_.equal_range(paren_state);
      for (auto cp_it = cp_begin; cp_it != cp_end; ++cp_it) {
        const auto& cparc = cp_it->second;
        const auto cpw =
...
#endif  // OPENFST_EXTENSIONS_PDT_SHORTEST_PATH_H_
