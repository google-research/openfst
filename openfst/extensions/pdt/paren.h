...
#ifndef OPENFST_EXTENSIONS_PDT_PAREN_H_
#define OPENFST_EXTENSIONS_PDT_PAREN_H_
...
class PdtParenReachable {
 public:
  ...
  using ParenMultimap = absl::flat_hash_map<StateId, std::vector<Label>>;

  // Maps from paren ID and state ID to reachable state set ID.
  using StateSetMap = absl::flat_hash_map<State, ssize_t, StateHash>;

  // Maps from paren ID and state ID to arcs exiting that state with that
  // Label.
  using ParenArcMultimap =
      absl::flat_hash_map<State, std::vector<Arc>, StateHash>;
...
class PdtBalanceData {
 public:
  ...
  // Set for open parens.
  using OpenParenSet = absl::flat_hash_set<State, StateHash>;

  // Maps from open paren destination state to parenthesis ID.
  using OpenParenMap = absl::flat_hash_map<StateId, std::vector<Label>>;

  // Maps from open paren state to source states of matching close parens
  using CloseParenMap =
      absl::flat_hash_map<State, std::vector<StateId>, StateHash>;

  // Maps from open paren state to close source set ID.
  using CloseSourceMap = absl::flat_hash_map<State, ssize_t, StateHash>;
...
PdtBalanceData<Arc>* PdtBalanceData<Arc>::Reverse(
  ...
  absl::flat_hash_set<StateId> close_sources;
...
#endif  // OPENFST_EXTENSIONS_PDT_PAREN_H_
