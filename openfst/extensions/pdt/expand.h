...
#ifndef OPENFST_EXTENSIONS_PDT_EXPAND_H_
#define OPENFST_EXTENSIONS_PDT_EXPAND_H_
...
#include "openfst/extensions/pdt/pdt.h"
#include "openfst/extensions/pdt/reverse.h"
...
class PdtPrunedExpand {
 ...
 private:
  ...
  void InitCloseParenMultimap(absl::Span<const std::pair<Label, Label>> parens);
  ...
  absl::flat_hash_map<StateId, Weight> dest_map_;
...
void PdtPrunedExpand<Arc>::InitCloseParenMultimap(
    absl::Span<const std::pair<Label, Label>> parens) {
  absl::flat_hash_map<Label, Label> paren_map;
...
void PdtPrunedExpand<Arc>::AddStateAndEnqueue(StateId s) {
  ...
  // TODO: Check everything is fine when kExpanded?
}
...
bool PdtPrunedExpand<Arc>::PruneArc(StateId s, const Arc& arc) {
  ...
  if ((cached_source_ != SourceState(s)) ||
    ...
    if (cached_source_ != ifst_->Start()) {
      ...
      // TODO: queue discipline should prevent this from ever
      // happening.
      // Replace by a check.
...
void PdtPrunedExpand<Arc>::ProcDestStates(StateId s, StackId si) {
  ...
  // TODO: clean up source state business; rename current function to
  // ProcSourceState.
...
#endif  // OPENFST_EXTENSIONS_PDT_EXPAND_H_
