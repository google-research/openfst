...
#ifndef OPENFST_EXTENSIONS_COMPRESS_COMPRESS_H_
...
class LempelZiv {
 public:
  ...
  void BatchEncode(absl::Span<const Edge> input,
                   std::vector<std::pair<Var, Edge>>* output);

  // Decodes codedvector to output, returning false if the index exceeds the
  // size.
  [[nodiscard]] bool BatchDecode(absl::Span<const std::pair<Var, Edge>> input,
...
void LempelZiv<Var, Edge, EdgeLessThan, EdgeEquals>::BatchEncode(
    absl::Span<const Edge> input, std::vector<std::pair<Var, Edge>>* output) {
...
class Compressor {
 public:
  ...
  [[nodiscard]] bool Decompress(std::istream& strm, absl::string_view source,
                                MutableFst<Arc>* fst);
...
template <class Arc>
[[nodiscard]] bool Decompress(absl::string_view source, MutableFst<Arc>* fst) {
...
#endif  // OPENFST_EXTENSIONS_COMPRESS_COMPRESS_H_
