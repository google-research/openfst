...
#ifndef OPENFST_EXTENSIONS_LINEAR_LINEAR_FST_H_
#define OPENFST_EXTENSIONS_LINEAR_LINEAR_FST_H_
...
class LinearTaggerFstImpl : public CacheImpl<A> {
 public:
  ...
  // tables.
  //
  // TODO: when there is no constraint on output we can delay
...
// TODO: this has much in duplicate with `ExpandArcs()`
template <class A>
...
class LinearClassifierFstImpl : public CacheImpl<A> {
 ...
 private:
  ...
  bool IsStartState(absl::Span<const Label> state) const {
  ...
  bool Write(const std::string& source) const override {
    if (!source.empty()) {
      file::FileOutStream strm(source,
                               std::ios_base::out | std::ios_base::binary);
      if (!strm) {
        LOG(ERROR) << "LinearClassifierFst::Write: Can't open file: " << source;
        return false;
      }
      return Write(strm, FstWriteOptions(source));
    } else {
...
#endif  // OPENFST_EXTENSIONS_LINEAR_LINEAR_FST_H_
