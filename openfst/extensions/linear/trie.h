...
#ifndef OPENFST_EXTENSIONS_LINEAR_TRIE_H_
#define OPENFST_EXTENSIONS_LINEAR_TRIE_H_
...
class NestedTrieTopology {
 public:
  typedef L Label;
  typedef H Hash;
  typedef absl::flat_hash_map<L, int, H> NextMap;
...
template <class L, class H>
inline typename NestedTrieTopology<L, H>::const_iterator&
NestedTrieTopology<L, H>::const_iterator::operator++(int) {
...
class FlatTrieTopology {
 private:
  typedef absl::flat_hash_map<ParentLabel<L>, int, ParentLabelHash<L, H>>
      NextMap;
...
#endif  // OPENFST_EXTENSIONS_LINEAR_TRIE_H_
