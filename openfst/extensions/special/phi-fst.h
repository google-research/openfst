...
#ifndef OPENFST_EXTENSIONS_SPECIAL_PHI_FST_H_
#define OPENFST_EXTENSIONS_SPECIAL_PHI_FST_H_
...
ABSL_DECLARE_FLAG(int64_t, phi_fst_phi_label);
ABSL_DECLARE_FLAG(bool, phi_fst_phi_loop);
ABSL_DECLARE_FLAG(std::string, phi_fst_rewrite_mode);
...
class PhiFstMatcherData {
 public:
  explicit PhiFstMatcherData(
      Label phi_label = absl::GetFlag(FLAGS_phi_fst_phi_label),
      bool phi_loop = absl::GetFlag(FLAGS_phi_fst_phi_loop),
      MatcherRewriteMode rewrite_mode =
          RewriteMode(absl::GetFlag(FLAGS_phi_fst_rewrite_mode)))
...
#endif  // OPENFST_EXTENSIONS_SPECIAL_PHI_FST_H_
