...
absl::Status RandFst(const int num_random_states, const int num_random_arcs,
                     ...
                     Generate generate, absl::BitGenRef bit_gen,
                     MutableFst<Arc>* fst) {
  ...
  const StateId ns = absl::Uniform(bit_gen, 0, num_random_states);
  if (ns == 0) return absl::OkStatus();
  fst->AddStates(ns);

  const StateId start = absl::Uniform(bit_gen, 0, ns);
  fst->SetStart(start);

  ArcDirection arc_direction = ANY_DIRECTION;
  if (absl::Bernoulli(bit_gen, acyclic_prob)) {
    arc_direction =
        absl::Bernoulli(bit_gen, .5) ? FORWARD_DIRECTION : REVERSE_DIRECTION;
  }

  if (num_random_arcs > 0) {
    const size_t na = absl::Uniform(bit_gen, 0, num_random_arcs);
    for (size_t n = 0; n < na; ++n) {
      StateId s = absl::Uniform(bit_gen, 0, ns);
      Arc arc;
      arc.ilabel = absl::Uniform(bit_gen, 0, num_random_labels);
      arc.olabel = absl::Uniform(bit_gen, 0, num_random_labels);
      arc.weight = generate(bit_gen);
      arc.nextstate = absl::Uniform(bit_gen, 0, ns);
  ...
  const StateId nf = absl::Uniform(absl::IntervalClosedClosed, bit_gen, 0, ns);
  for (StateId n = 0; n < nf; ++n) {
    const StateId s = absl::Uniform(bit_gen, 0, ns);
    fst->SetFinal(s, generate(bit_gen));
  }
  ...
  uint64_t mask = 0;
  for (int n = 0; n < 8; ++n) {
    mask |= absl::Uniform<uint8_t>(bit_gen);
    mask <<= 8;
  }
