...
template <class T>
void TestTemplatedWeights(absl::BitGenRef bit_gen, int repeat) {
  WeightGenerate<TropicalWeightTpl<T>> tropical_generate;
  WeightTester<TropicalWeightTpl<T>> tropical_tester(tropical_generate);
  tropical_tester.Test(bit_gen, repeat);

  WeightGenerate<LogWeightTpl<T>> log_generate;
  WeightTester<LogWeightTpl<T>> log_tester(log_generate);
  log_tester.Test(bit_gen, repeat);

  WeightGenerate<RealWeightTpl<T>> real_generate;
  WeightTester<RealWeightTpl<T>> real_tester(real_generate);
  real_tester.Test(bit_gen, repeat);

  WeightGenerate<MinMaxWeightTpl<T>> minmax_generate(true);
  WeightTester<MinMaxWeightTpl<T>> minmax_tester(minmax_generate);
  minmax_tester.Test(bit_gen, repeat);

  WeightGenerate<SignedLogWeightTpl<T>> signedlog_generate(true);
  WeightTester<SignedLogWeightTpl<T>> signedlog_tester(signedlog_generate);
  signedlog_tester.Test(bit_gen, repeat);
}
...
void RunTest() {
  ...
  TestTemplatedWeights<double>(bit_gen, absl::GetFlag(FLAGS_repeat));
  ...
  left_string_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  ...
  WeightGenerate<IUSetWeight> iu_set_generate;
  WeightTester<IUSetWeight> iu_set_tester(iu_set_generate);
  iu_set_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));

  using UISetWeight = SetWeight<int, SET_UNION_INTERSECT>;
  WeightGenerate<UISetWeight> ui_set_generate;
  WeightTester<UISetWeight> ui_set_tester(ui_set_generate);
  ui_set_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));

  // SET_INTERSECT_UNION_RESTRICT not tested since it requires equal sets,
  // so would fail.

  using BoolSetWeight = SetWeight<int, SET_BOOLEAN>;
  WeightGenerate<BoolSetWeight> bool_set_generate;
  WeightTester<BoolSetWeight> bool_set_tester(bool_set_generate);
  bool_set_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));

  TestWeightConversion<IUSetWeight, UISetWeight>(iu_set_generate(bit_gen));

  TestWeightCopy<IUSetWeight, UISetWeight>(iu_set_generate(bit_gen));
  TestWeightCopy<IUSetWeight, BoolSetWeight>(iu_set_generate(bit_gen));
  TestWeightCopy<UISetWeight, IUSetWeight>(ui_set_generate(bit_gen));
  TestWeightCopy<UISetWeight, BoolSetWeight>(ui_set_generate(bit_gen));
  TestWeightCopy<BoolSetWeight, IUSetWeight>(bool_set_generate(bit_gen));
  TestWeightCopy<BoolSetWeight, UISetWeight>(bool_set_generate(bit_gen));

  TestWeightMove<IUSetWeight, UISetWeight>(iu_set_generate(bit_gen));
  TestWeightMove<IUSetWeight, BoolSetWeight>(iu_set_generate(bit_gen));
  TestWeightMove<UISetWeight, IUSetWeight>(ui_set_generate(bit_gen));
  TestWeightMove<UISetWeight, BoolSetWeight>(ui_set_generate(bit_gen));
  TestWeightMove<BoolSetWeight, IUSetWeight>(bool_set_generate(bit_gen));
  TestWeightMove<BoolSetWeight, UISetWeight>(bool_set_generate(bit_gen));

  // COMPOSITE WEIGHTS AND TESTERS - DEFINITIONS

  using TropicalGallicWeight = GallicWeight<int, TropicalWeight>;
  WeightGenerate<TropicalGallicWeight> tropical_gallic_generate(true);
  ...
  WeightGenerate<TropicalGenGallicWeight> tropical_gen_gallic_generate(false);
  ...
  // Unnested composite.
  tropical_gallic_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  tropical_gen_gallic_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  tropical_product_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  tropical_lexicographic_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  tropical_cube_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  log_sparse_power_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  log_log_expectation_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  real_real_expectation_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  tropical_union_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));

  // Nested composite.
  first_nested_product_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  second_nested_product_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  nested_product_cube_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  sparse_nested_product_cube_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
  log_log_sparse_expectation_tester.Test(bit_gen, absl::GetFlag(FLAGS_repeat));
