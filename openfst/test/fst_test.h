...
#ifndef OPENFST_TEST_FST_TEST_H_
...
class FstTester {
 public:
  ...
  void TestBase(const G& fst) const {
    ...
    EXPECT_EQ(fst.Start(), has_states ? 0 : kNoStateId);
    for (siter.Reset(); !siter.Done(); siter.Next()) {
      StateId s = siter.Value();
      matcher.SetState(s);
      EXPECT_EQ(fst.Final(s), NthWeight(s));
      size_t na = 0;
      ...
      for (aiter.Reset(); !aiter.Done(); aiter.Next()) {
        ...
        EXPECT_EQ(arc.ilabel, na);
        EXPECT_EQ(arc.olabel, 0);
        EXPECT_EQ(arc.weight, NthWeight(na));
        if (na == ns + 1) {
          EXPECT_EQ(arc.nextstate, s == num_states_ - 1 ? 0 : s + 1);
        } else {
          EXPECT_EQ(arc.nextstate, s);
        }
        if (match_type == MATCH_INPUT) {
          ASSERT_TRUE(matcher.Find(arc.ilabel));
          EXPECT_EQ(matcher.Value().ilabel, arc.ilabel);
        }
      }
      EXPECT_EQ(na, s + 1);
      EXPECT_EQ(na, aiter.Position());
      EXPECT_EQ(fst.NumArcs(s), s + 1);
      EXPECT_EQ(fst.NumInputEpsilons(s), 0);
      EXPECT_EQ(fst.NumOutputEpsilons(s), s + 1);
      EXPECT_FALSE(matcher.Find(s + 2));     // out-of-range
      EXPECT_FALSE(matcher.Find(kNoLabel));  // no explicit input epsilons
      ASSERT_TRUE(matcher.Find(0));
      EXPECT_EQ(matcher.Value().ilabel, kNoLabel);  // implicit epsilon loop
      ++ns;
    }
    EXPECT_EQ(num_states_, ns);
    EXPECT_TRUE(Verify(fst));
    EXPECT_TRUE(fst.Properties(ns > 0 ? kNotAcceptor : kAcceptor, true));
    EXPECT_TRUE(fst.Properties(ns > 0 ? kOEpsilons : kNoOEpsilons, true));
  }
  ...
  void TestExpanded(const G& fst) const {
    EXPECT_EQ(fst.NumStates(), num_states_);
    StateId ns = 0;
    for (StateIterator<G> siter(fst); !siter.Done(); siter.Next()) {
      ++ns;
    }
    EXPECT_EQ(fst.NumStates(), ns);
    EXPECT_TRUE(fst.Properties(kExpanded, false));
  }
  ...
  void TestMutable(G* fst) const {
    for (StateIterator<G> siter(*fst); !siter.Done(); siter.Next()) {
      ...
      for (aiter.Reset(); !aiter.Done(); aiter.Next()) {
        ...
        EXPECT_EQ(fst->NumInputEpsilons(s), ni + 1);
        arc.ilabel = na;
        aiter.SetValue(arc);
        EXPECT_EQ(fst->NumInputEpsilons(s), ni);
      }
    }
    ...
    for (StateIterator<G> siter(*cfst2); !siter.Done(); siter.Next()) {
      StateId s = siter.Value();
      cfst2->DeleteArcs(s);
      EXPECT_EQ(cfst2->NumArcs(s), 0);
      EXPECT_EQ(cfst2->NumInputEpsilons(s), 0);
      EXPECT_EQ(cfst2->NumOutputEpsilons(s), 0);
    }
  }
  ...
  void TestAssign(const G& fst) const {
    ...
    G afst1;
    afst1 = fst;
    EXPECT_TRUE(Equal(fst, afst1));

    // Assignment from Fst
    G afst2;
    afst2 = static_cast<const Fst<Arc>&>(fst);
    EXPECT_TRUE(Equal(fst, afst2));
  ...
  void TestIO(const G& fst) const {
        ...
        JoinPath(::testing::TempDir(), "aligned.fst");
    {
      ...
      auto ffst = absl::WrapUnique(G::Read(filename));
      ASSERT_TRUE(ffst);
      TestBase(*ffst);
    }

    {
      ...
      auto gfst = absl::WrapUnique(Fst<Arc>::Read(filename));
      ASSERT_TRUE(gfst);
      G* dfst = absl::down_cast<G*>(gfst.get());
      TestBase(*dfst);

      // generic write/read/test
      ASSERT_TRUE(gfst->Write(filename));
      auto hfst = absl::WrapUnique(Fst<Arc>::Read(filename));
      ASSERT_TRUE(hfst);
      TestBase(*hfst);
    }

    {
      ...
      opts.source = aligned;
      auto gfst = absl::WrapUnique(G::Read(istr, opts));
      ASSERT_TRUE(gfst);
      TestBase(*gfst);
    }
    ...
    if (fst.Properties(kExpanded, false)) {
      auto efst = absl::WrapUnique(ExpandedFst<Arc>::Read(filename));
      ASSERT_TRUE(efst);
      TestBase(*efst);
      TestExpanded(*efst);
    }
    ...
    if (fst.Properties(kMutable, false)) {
      auto mfst = absl::WrapUnique(MutableFst<Arc>::Read(filename));
      ASSERT_TRUE(mfst);
      TestBase(*mfst);
      TestExpanded(*mfst);
...
#endif  // OPENFST_TEST_FST_TEST_H_
