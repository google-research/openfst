...
// Movable seed sequence wrapper and factory for PRNG seeding.
...
// A custom std::seed_seq wrapper that is movable, allowing it to be returned
...
// Construct a seed sequence from a descriptive tag string. The tag is
// combined with a salt (from --seed_seq_salt if set, otherwise from
// std::random_device) to seed the sequence. When no flag is provided, the
// generated salt is logged to stderr so it can be recovered for replay.
inline SeedSeq MakeTaggedSeedSeq(absl::string_view tag) { return SeedSeq(tag); }
