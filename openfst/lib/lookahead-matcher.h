...
#ifndef OPENFST_LIB_LOOKAHEAD_MATCHER_H_
#define OPENFST_LIB_LOOKAHEAD_MATCHER_H_
...
ABSL_DECLARE_FLAG(std::string, save_relabel_ipairs);
ABSL_DECLARE_FLAG(std::string, save_relabel_opairs);
...
void RelabelForReachable(FST* fst, const Data& data,
                         absl::string_view save_relabel_ipairs,
                         absl::string_view save_relabel_opairs) {
...
inline LabelLookAheadRelabeler<Arc, Data>::LabelLookAheadRelabeler(
  ...
  if (is_mutable) {
    ...
    mfst.reset(absl::down_cast<MutableFst<Arc>*>(&fst));
  } else {
    mfst = std::make_unique<VectorFst<Arc>>(fst);
  }

  RelabelForReachable<Reachable>(mfst.get(), *data,
                                 absl::GetFlag(FLAGS_save_relabel_ipairs),
                                 absl::GetFlag(FLAGS_save_relabel_opairs));
...
class LookAheadMatcher {
 public:
  ...
  bool LookAheadLabel(Label label) const {
    if (LookAheadCheck()) {
      return absl::down_cast<LBase*>(base_.get())->LookAheadLabel(label);
    } else {
      return true;
    }
  }
  ...
  bool LookAheadFst(const Fst<Arc>& fst, StateId s) {
    if (LookAheadCheck()) {
      return absl::down_cast<LBase*>(base_.get())->LookAheadFst(fst, s);
    } else {
      return true;
    }
  }
  ...
  Weight LookAheadWeight() const {
    if (LookAheadCheck()) {
      return absl::down_cast<LBase*>(base_.get())->LookAheadWeight();
    } else {
      return Weight::One();
    }
  }
  ...
  bool LookAheadPrefix(Arc* arc) const {
    if (LookAheadCheck()) {
      return absl::down_cast<LBase*>(base_.get())->LookAheadPrefix(arc);
    } else {
      return false;
    }
  }
  ...
  void InitLookAheadFst(const Fst<Arc>& fst, bool copy = false) {
    if (LookAheadCheck()) {
      absl::down_cast<LBase*>(base_.get())->InitLookAheadFst(fst, copy);
    }
  }
...
#endif  // OPENFST_LIB_LOOKAHEAD_MATCHER_H_
