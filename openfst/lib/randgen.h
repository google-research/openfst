...
#ifndef OPENFST_LIB_RANDGEN_H_
#define OPENFST_LIB_RANDGEN_H_
 ...
 public:
  ...
  size_t operator()(const Fst<Arc>& fst, StateId s) const {
    ...
    for (aiter.Reset(); !aiter.Done(); aiter.Next(), ++n) {
      ...
      if (p.Value() < log_threshold) return n;
    }
    return n;
  }
...
void OneMultinomialSample(absl::Span<const double> probs, size_t num_to_sample,
  ...
  absl::FixedArray<double> norm(probs.size());
