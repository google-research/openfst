...
MappedFile* absl_nullable MappedFile::Map(std::istream& istrm, bool memorymap,
  ...
  if (memorymap) {
    LOG(INFO) << "File mapping at offset " << spos << " of file " << source
...
MappedFile* absl_nullable MappedFile::MapFromFileDescriptor(int fd, size_t pos,
                                                            size_t size) {
#ifdef _WIN32
  SYSTEM_INFO sysInfo;
...
MappedFile* MappedFile::Allocate(size_t size, size_t align) {
  ...
  if (size > 0) {
    region.offset = align;
    region.data = static_cast<char*>(operator new(size, std::align_val_t{align}
                                                  ));
  }
  region.mmap = nullptr;
  region.size = size;
