...
ABSL_DECLARE_FLAG(bool, fst_error_fatal);

#define FSTERROR()                                                           \
  LOG(LEVEL(absl::GetFlag(FLAGS_fst_error_fatal) ? absl::LogSeverity::kFatal \
...
inline std::ostream& WriteType(std::ostream& strm, absl::string_view s) {
  int32_t ns = s.size();
  WriteType(strm, ns);
...
// consumed during integer parsing, otherwise returns `absl::nullopt`.
std::optional<int64_t> ParseInt64(absl::string_view s, int base = 10);

int64_t StrToInt64(absl::string_view s, absl::string_view source, size_t nline,
                   bool* absl_nullable error = nullptr);
...
template <typename I>
bool ReadIntPairs(absl::string_view source,
                  std::vector<std::pair<I, I>>* pairs) {
  file::FileInStream strm(std::string(source), std::ios_base::in);
  if (!strm) {
...
template <typename I>
bool WriteIntPairs(absl::string_view source,
...
bool ReadLabelPairs(absl::string_view source,
                    std::vector<std::pair<Label, Label>>* pairs) {
  return ReadIntPairs(source, pairs);
}

template <typename Label>
bool WriteLabelPairs(absl::string_view source,
...
// Increase buffer position to an equivalent state as the ostream. This assumes
// the buffer position is at 0. Returns the offset applied to the buffer
// position, or -1 if there was an error.
