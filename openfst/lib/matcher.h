...
#ifndef OPENFST_LIB_MATCHER_H_
#define OPENFST_LIB_MATCHER_H_
...
#include "openfst/lib/mutable-fst.h"  // for all internal FST accessors.
...
class SortedMatcher : public MatcherBase<typename F::Arc> {
 public:
  ...
  // o.w. linear search is used.
  // This doesn't copy the FST.
  SortedMatcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
...
class HashMatcher : public MatcherBase<typename F::Arc> {
 public:
  ...
  // This doesn't copy the FST.
  HashMatcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
...
class PhiMatcher : public MatcherBase<typename M::Arc> {
 public:
  ...
  // This doesn't copy the FST.
  PhiMatcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
...
class RhoMatcher : public MatcherBase<typename M::Arc> {
 public:
  ...
  // This doesn't copy the FST.
  RhoMatcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
...
class SigmaMatcher : public MatcherBase<typename M::Arc> {
 public:
  ...
  // This doesn't copy the FST.
  SigmaMatcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
...
class MultiEpsMatcher {
 public:
  ...
  // This doesn't copy the FST.
  MultiEpsMatcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
                  MatchType match_type,
                  uint32_t flags = (kMultiEpsLoop | kMultiEpsList),
                  M* matcher = nullptr, bool own_matcher = true)
      : matcher_(matcher ? matcher : new M(fst, match_type)),
...
class ExplicitMatcher : public MatcherBase<typename M::Arc> {
 public:
  ...
  // This doesn't copy the FST.
  ExplicitMatcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
...
class Matcher {
 public:
  ...
  // This doesn't copy the FST.
  Matcher(const FST* absl_nonnull fst ABSL_ATTRIBUTE_LIFETIME_BOUND,
          MatchType match_type)
  ...
  const FST& GetFst() const {
    return absl::down_cast<const FST&>(base_->GetFst());
  }
