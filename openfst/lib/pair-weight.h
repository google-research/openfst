...
class PairWeight {
 public:
  ...
  // We hash the results of Hash() instead of the objects directly because
  // OpenFst weight types do not yet support absl::Hash (they lack
  // AbslHashValue).
...
class WeightGenerate<PairWeight<W1, W2>> {
 public:
  ...
  Weight operator()(absl::BitGenRef bit_gen) const {
    return Weight(generate1_(bit_gen), generate2_(bit_gen));
  }
...
#endif  // OPENFST_LIB_PAIR_WEIGHT_H_
