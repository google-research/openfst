...
#ifndef OPENFST_LIB_ADD_ON_H_
#define OPENFST_LIB_ADD_ON_H_
...
class AddOnImpl : public FstImpl<typename FST::Arc> {
 public:
  ...
  AddOnImpl(const FST& fst, absl::string_view type,
  ...
  // implementation.
  AddOnImpl(const Fst<Arc>& fst, absl::string_view type,
            std::shared_ptr<T> t = nullptr)
  ...
  static AddOnImpl* Read(std::istream& strm, const FstReadOptions& opts) {
    ...
    auto impl = absl::WrapUnique(new AddOnImpl(nopts.header->FstType()));
