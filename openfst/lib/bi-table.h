...
#ifndef OPENFST_LIB_BI_TABLE_H_
#define OPENFST_LIB_BI_TABLE_H_
...
// Default hash set is std::unordered_set.
...
struct HashSet<K, H, E, HS_FLAT> : public absl::flat_hash_set<K, H, E> {
 private:
  using Base = absl::flat_hash_set<K, H, E>;
...
class VectorHashBiTable {
 ...
 private:
  ...
  H h_;         // Hash function used for hashing into `keys_`.
