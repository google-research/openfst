...
#ifndef OPENFST_LIB_LABEL_REACHABLE_H_
#define OPENFST_LIB_LABEL_REACHABLE_H_
...
class LabelReachableData {
 public:
  ...
  static LabelReachableData* Read(std::istream& istrm,
    ...
    auto data = absl::WrapUnique(new LabelReachableData());
...
bool StateSort(std::vector<IntervalSet<Label>>* interval_sets,
  ...
  std::vector<IntervalSet<Label>> reordered_interval_sets(
      interval_sets->size());
  // TODO: Use storage-efficient cycle-following algorithm
...
#endif  // OPENFST_LIB_LABEL_REACHABLE_H_
