...
bool ConvertSymbolToLabel(absl::string_view str, const SymbolTable* syms,
...
bool ConvertStringToLabels(
    absl::string_view str, TokenType token_type, const SymbolTable* syms,
  ...
  switch (token_type) {
    ...
    case TokenType::SYMBOL: {
      const std::string separator = absl::StrCat("\n", sep);
      for (absl::string_view c :
           absl::StrSplit(str, absl::ByAnyChar(separator), absl::SkipEmpty())) {
        Label label;
...
class StringCompiler {
 public:
  ...
  template <class FST>
  bool operator()(
      absl::string_view str, FST* fst, Weight weight,
      absl::string_view sep = absl::GetFlag(FLAGS_fst_field_separator)) const {
    std::vector<Label> labels;
    if (!internal::ConvertStringToLabels(str, token_type_, syms_,
                                         unknown_label_, &labels, sep)) {
