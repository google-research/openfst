...
#ifndef OPENFST_LIB_FST_H_
#define OPENFST_LIB_FST_H_
...
#include "openfst/lib/port.h"
#include "openfst/lib/properties.h"
#include "openfst/lib/register.h"
#include "openfst/lib/symbol-table.h"
#include "openfst/lib/util.h"

ABSL_DECLARE_FLAG(bool, fst_align);
...
struct FstReadOptions {
  ...
  bool verify;                  // Perform FST type-specific light-weight sanity
                                // check, where supported by implementations.
                                // (default: true).

  explicit FstReadOptions(absl::string_view source = "<unspecified>",
                          const FstHeader* absl_nullable header = nullptr,
                          const SymbolTable* absl_nullable isymbols = nullptr,
                          const SymbolTable* absl_nullable osymbols = nullptr);

  explicit FstReadOptions(absl::string_view source, const SymbolTable* isymbols,
                          const SymbolTable* osymbols = nullptr);

  // Helper function to convert strings FileReadModes into their enum value.
  static FileReadMode ReadMode(absl::string_view mode);
...
struct FstWriteOptions {
  ...
  explicit FstWriteOptions(absl::string_view source = "<unspecified>",
                           bool write_header = true, bool write_isymbols = true,
                           bool write_osymbols = true,
                           bool align = absl::GetFlag(FLAGS_fst_align),
...
class FstHeader {
 public:
  ...
  void SetFstType(absl::string_view type) { fsttype_ = std::string(type); }

  void SetArcType(absl::string_view type) { arctype_ = std::string(type); }
  ...
  bool Write(std::ostream& strm, absl::string_view source) const;
...
class FST_LTO_VISIBILITY_PUBLIC Fst {
 public:
  ...
  static Fst* Read(absl::string_view source) {
    if (!source.empty()) {
...
class FstImplBase {
 private:
  ...
  FstImplBase& operator=(const FstImplBase& impl) {
    ...
    type_ = impl.type_;
    isymbols_ =
        absl::WrapUnique(impl.isymbols_ ? impl.isymbols_->Copy() : nullptr);
    osymbols_ =
        absl::WrapUnique(impl.osymbols_ ? impl.osymbols_->Copy() : nullptr);
    return *this;
  }
...
class FST_LTO_VISIBILITY_PUBLIC FstImpl : public FstImplBase {
 public:
  ...
  static void WriteFstHeader(const Fst<Arc>& fst, std::ostream& strm,
                             ...
                             absl::string_view type, uint64_t properties,
                             FstHeader* hdr) {
    if (opts.write_header) {
      hdr->SetFstType(type);
      hdr->SetArcType(Arc::Type());
