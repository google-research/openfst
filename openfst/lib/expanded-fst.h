...
#ifndef OPENFST_LIB_EXPANDED_FST_H_
#define OPENFST_LIB_EXPANDED_FST_H_
...
class ImplToExpandedFst : public ImplToFst<I, FST> {
 ...
 protected:
  ...
  // Empty source reads from standard input.
  static Impl* Read(absl::string_view source) {
    if (!source.empty()) {
      file::FileInStream strm(std::string(source),
                              std::ios_base::in | std::ios_base::binary);
      if (!strm) {
...
#endif  // OPENFST_LIB_EXPANDED_FST_H_
