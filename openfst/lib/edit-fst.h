...
#ifndef OPENFST_LIB_EDIT_FST_H_
#define OPENFST_LIB_EDIT_FST_H_
...
class EditFstImpl : public FstImpl<A> {
 public:
  ...
  explicit EditFstImpl(const Fst<Arc>& wrapped)
      : wrapped_(absl::down_cast<WrappedFstT*>(wrapped.Copy())) {
    FstImpl<Arc>::SetType("edit");
    data_ = std::make_shared<EditFstData<Arc, WrappedFstT, MutableFstT>>();
  ...
  EditFstImpl(const EditFstImpl& impl)
      : FstImpl<Arc>(),
        wrapped_(absl::down_cast<WrappedFstT*>(impl.wrapped_->Copy(true))),
        data_(impl.data_) {
...
EditFstImpl<Arc, WrappedFstT, MutableFstT>::Read(std::istream& strm,
  ...
  impl->wrapped_.reset(absl::down_cast<WrappedFstT*>(wrapped_fst.release()));
...
class EditFst : public ImplToMutableFst<
 ...
 public:
  ...
  static EditFst* Read(absl::string_view source) {
