...
#ifndef OPENFST_LIB_MUTABLE_FST_H_
#define OPENFST_LIB_MUTABLE_FST_H_
...
class MutableFst : public ExpandedFst<A> {
 public:
  ...
  static MutableFst* Read(std::istream& strm, const FstReadOptions& opts) {
    ...
    return absl::down_cast<MutableFst*>(fst);
  }
  ...
  static MutableFst* Read(const std::string& source, bool convert = false,
                          absl::string_view convert_type = "vector") {
    if (convert == false) {
      if (!source.empty()) {
      ...
      if (ifst->Properties(kMutable, false)) {
        return absl::down_cast<MutableFst*>(ifst.release());
      } else {
        ...
        return absl::down_cast<MutableFst*>(ofst.release());
      }
    }
  }
...
#endif  // OPENFST_LIB_MUTABLE_FST_H_
