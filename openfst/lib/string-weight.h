...
#ifndef OPENFST_LIB_STRING_WEIGHT_H_
#define OPENFST_LIB_STRING_WEIGHT_H_
...
inline std::istream& operator>>(std::istream& strm,
  ...
  if (str == "Infinity") {
    ...
    weight.Clear();
    for (absl::string_view sv : absl::StrSplit(str, kStringSeparator)) {
...
struct GallicWeight
  ...
  explicit GallicWeight(absl::string_view s, int* nread = nullptr)
...
struct GallicWeight<Label, W, GALLIC>
  ...
  explicit GallicWeight(absl::string_view str, int* nread = nullptr)
      : UW(str, nread) {}
...
#endif  // OPENFST_LIB_STRING_WEIGHT_H_
