...
#ifndef OPENFST_LIB_SYMBOL_TABLE_H_
#define OPENFST_LIB_SYMBOL_TABLE_H_
...
ABSL_DECLARE_FLAG(bool, fst_compat_symbols);
ABSL_DECLARE_FLAG(std::string, fst_field_separator);
...
class DenseSymbolMap {
 public:
  ...
  std::pair<int64_t, bool> InsertOrFind(absl::string_view key);

  int64_t Find(absl::string_view key) const;
 ...
 private:
  ...
  size_t GetHash(absl::string_view key) const {
    return str_hash_(key) & hash_mask_;
  }

  const absl::Hash<absl::string_view> str_hash_;
...
class SymbolTableImplBase {
 public:
  ...
  virtual int64_t AddSymbol(absl::string_view symbol, int64_t key) = 0;

  virtual int64_t AddSymbol(absl::string_view symbol) = 0;
  ...
  virtual int64_t Find(absl::string_view symbol) const = 0;

  virtual bool Member(int64_t key) const { return !Find(key).empty(); }

  virtual bool Member(absl::string_view symbol) const {
  ...
  virtual void SetName(absl::string_view new_name) = 0;
...
class ConstSymbolTableImpl : public SymbolTableImplBase {
 public:
  absl_nullable std::unique_ptr<SymbolTableImplBase> Copy() const final;

  int64_t AddSymbol(absl::string_view symbol, int64_t key) final;

  int64_t AddSymbol(absl::string_view symbol) final;

  void RemoveSymbol(int64_t key) final;

  void SetName(absl::string_view new_name) final;
  void AddTable(const SymbolTable& table) final;

  bool IsMutable() const final { return false; }
};

// Default SymbolTable implementation using DenseSymbolMap and absl::btree_map.
// Provides the common text and binary format serialization.
class SymbolTableImpl final : public MutableSymbolTableImpl {
 public:
  explicit SymbolTableImpl(absl::string_view name)
      : name_(name),
        available_key_(0),
        dense_key_limit_(0),
  ...
  int64_t AddSymbol(absl::string_view symbol, int64_t key) override;

  int64_t AddSymbol(absl::string_view symbol) override {
  ...
  static SymbolTableImpl* absl_nullable ReadText(
      std::istream& strm, absl::string_view name,
  ...
  int64_t Find(absl::string_view symbol) const override {
  ...
  void SetName(absl::string_view new_name) override {
...
class SymbolTable {
 public:
  class iterator {
   public:
    // TODO: Expand `SymbolTable::iterator` to be a random access
    // iterator.
  ...
  explicit SymbolTable(absl::string_view name = "<unspecified>")
  ...
  static SymbolTable* absl_nullable ReadText(const std::string& source,
                                             absl::string_view sep = "");

  // Reads a binary dump of the symbol table from a stream.
  static SymbolTable* Read(std::istream& strm, absl::string_view source) {
    auto impl = absl::WrapUnique(internal::SymbolTableImpl::Read(strm, source));
    return impl ? new SymbolTable(std::move(impl)) : nullptr;
  }
  ...
  int64_t AddSymbol(absl::string_view symbol, int64_t key) {
    MutateCheck();
  ...
  int64_t AddSymbol(absl::string_view symbol) {
    MutateCheck();
  ...
  int64_t Find(absl::string_view symbol) const { return impl_->Find(symbol); }
  ...
  bool Member(absl::string_view symbol) const { return impl_->Member(symbol); }
  ...
  void SetName(absl::string_view new_name) {
    MutateCheck();
  ...
  absl::Status WriteTextWithStatus(
      std::ostream& strm,
      // Characters to be used as a separator between fields in a textual
      // `SymbolTable` file, encoded as a string. Each byte in the string is
      // considered a valid separator. Multi-byte separators are not permitted.
      // If empty, will use the value of `FLAGS_fst_field_separator` that by
      // the default is set to "\t " (will output tab).
      absl::string_view sep = "") const;

  // Dumps a text representation of the symbol table to a file at the given
  // path. If `path` is empty, writes to stdout.
  absl::Status WriteTextWithStatus(const std::string& path,
                                   absl::string_view sep = "") const;

  // clang-format off
  [[deprecated("Use `WriteTextWithStatus` instead.")]] ABSL_REFACTOR_INLINE
  bool WriteText(std::ostream& strm, absl::string_view sep = "") const {
    return WriteTextWithStatus(strm, sep).ok();
  }

  [[deprecated("Use `WriteTextWithStatus` instead.")]] ABSL_REFACTOR_INLINE
  bool WriteText(const std::string& sink, absl::string_view sep = "") const {
    return WriteTextWithStatus(sink, sep).ok();
  }
  // clang-format on
 ...
 protected:
  ...
  const T* Impl() const {
    return absl::down_cast<const T*>(impl_.get());
  }

  template <class T = internal::SymbolTableImplBase>
  T* MutableImpl() {
    MutateCheck();
    return absl::down_cast<T*>(impl_.get());
  }
...
//
// TODO: consider adding options to allow for some form of implicit
// identity relabeling.
...
#endif  // OPENFST_LIB_SYMBOL_TABLE_H_
