...
#ifndef OPENFST_LIB_ENCODE_H_
#define OPENFST_LIB_ENCODE_H_
...
class EncodeTableHeader {
 public:
  ...
  void SetArcType(absl::string_view arctype) {
  ...
  bool Read(std::istream& strm, absl::string_view source);
...
class EncodeTable {
 public:
  ...
  class TripleHash {
   public:
    ...
    size_t operator()(const Triple* triple) const {
      // TODO: the equality operator unconditionally compares
      // olabel and weight fields so conditionally not using them here just
      // degrades the performance of the hash table by causing more collisions.
      return absl::HashOf(
          triple->ilabel, (flags_ & kEncodeLabels) ? triple->olabel : 0,
          (flags_ & kEncodeWeights) ? triple->weight.Hash() : 0);
    }
  ...
  static EncodeTable* Read(std::istream& strm, absl::string_view source);
...
EncodeTable<Arc>* EncodeTable<Arc>::Read(std::istream& strm,
  ...
  for (int64_t i = 0; i < size; ++i) {
    table->triples_.push_back(Triple::Read(strm));
...
class EncodeMapper {
 ...
 public:
  ...
  static EncodeMapper* Read(std::istream& strm, absl::string_view source,
  ...
  static EncodeMapper* Read(absl::string_view source,
                            EncodeType type = ENCODE) {
  ...
  bool Write(absl::string_view source) const {
    file::FileOutStream strm(std::string(source),
