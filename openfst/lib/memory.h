...
#ifndef OPENFST_LIB_MEMORY_H_
#define OPENFST_LIB_MEMORY_H_
...
class MemoryPoolImpl : public MemoryPoolBase {
 public:
  ...
  void* Allocate() {
    ...
    return absl::implicit_cast<std::byte*>(link->buf);
  }
...
// See openfst/lib/visit.h (global new) and
// openfst/lib/dfs-visit.h (class new) for examples.
template <typename T>
...
class MemoryArenaCollection {
 public:
  ...
  MemoryArena<T>* Arena() {
    ...
    return absl::down_cast<MemoryArena<T>*>(arena.get());
  }
...
class MemoryPoolCollection {
 public:
  ...
  MemoryPool<T>* Pool() {
    ...
    return absl::down_cast<MemoryPool<T>*>(pool.get());
  }
