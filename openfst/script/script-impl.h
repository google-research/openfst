...
class GenericOperationRegister
    : public GenericRegister<std::pair<absl::string_view, absl::string_view>,
                             OperationSignature,
                             GenericOperationRegister<OperationSignature>> {
 public:
  OperationSignature GetOperation(absl::string_view operation_name,
                                  absl::string_view arc_type) {
    return this->GetEntry(std::make_pair(operation_name, arc_type));
  }

 protected:
  std::string ConvertKeyToSoFilename(
      const std::pair<absl::string_view, absl::string_view>& key) const final {
...
//
// The extern "C" force_link variable is used to force the MSVC linker to
// include the object file containing the registration code. The /include
// pragma requires an un-mangled symbol name, so we use an extern "C"
// variable rather than the Registerer object itself.
#ifdef _MSC_VER
#define OPENFST_FORCE_LINK_VAR(var_name) \
  extern "C" int var_name;               \
  int var_name = 0;                      \
...
#define REGISTER_FST_OPERATION(Op, Arc, ArgPack)                         \
  ...
  OPENFST_FORCE_LINK_VAR(                                                \
      arc_dispatched_operation_##ArgPack##Op##Arc##_force_link);
...
void CopyWeights(absl::Span<const WeightClass> weights,
...
inline std::vector<std::pair<int64_t, const FstClass*>> BorrowPairs(
    absl::Span<const std::pair<int64_t, std::unique_ptr<const FstClass>>>
        pairs) {
...
#endif  // OPENFST_SCRIPT_SCRIPT_IMPL_H_
