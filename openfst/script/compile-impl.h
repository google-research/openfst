...
class FstCompiler {
 public:
  ...
  FstCompiler(std::istream& istrm, absl::string_view source,
  ...
  FstCompiler(std::istream& istrm, absl::string_view source, SymbolTable* isyms,
  ...
  void Init(std::istream& istrm, absl::string_view source, SymbolTable* isyms,
            SymbolTable* osyms, SymbolTable* ssyms, bool accep, bool ikeep,
            bool okeep, bool nkeep, bool add_symbols) {
    nline_ = 0;
 ...
 private:
  ...
  StateId StrToId(absl::string_view s, SymbolTable* syms,
  ...
  StateId StrToStateId(absl::string_view s) {
  ...
  StateId StrToILabel(absl::string_view s) const {
    return StrToId(s, isyms_, "arc ilabel");
  }

  StateId StrToOLabel(absl::string_view s) const {
  ...
  absl::flat_hash_map<StateId, StateId> states_;  // State ID map.
...
#endif  // OPENFST_SCRIPT_COMPILE_IMPL_H_
