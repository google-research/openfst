...
#ifndef OPENFST_SCRIPT_WEIGHT_CLASS_H_
#define OPENFST_SCRIPT_WEIGHT_CLASS_H_
...
class WeightImplBase {
 public:
  ...
  virtual uint64_t Properties() const = 0;
...
class WeightClassImpl : public WeightImplBase {
 public:
  ...
  std::string ToString() const final { return WeightToStr(weight_); }

  bool Member() const final { return weight_.Member(); }

  uint64_t Properties() const override { return weight_.Properties(); }

  bool operator==(const WeightImplBase& other) const final {
    const auto* typed_other =
        absl::down_cast<const WeightClassImpl<W>*>(&other);
...
class WeightClass {
 public:
  ...
  WeightClass(absl::string_view weight_type, absl::string_view weight_str);
  ...
  static constexpr absl::string_view __ZERO__ = "__ZERO__";
  static constexpr absl::string_view __ONE__ = "__ONE__";
  static constexpr absl::string_view __NOWEIGHT__ = "__NOWEIGHT__";

  static WeightClass Zero(absl::string_view weight_type);

  static WeightClass One(absl::string_view weight_type);

  static WeightClass NoWeight(absl::string_view weight_type);
  ...
  uint64_t Properties() const { return impl_ ? impl_->Properties() : 0; }
...
using StrToWeightImplBaseT =
    std::unique_ptr<WeightImplBase> (*)(absl::string_view str);

template <class W>
std::unique_ptr<WeightImplBase> StrToWeightImplBase(absl::string_view str) {
...
class WeightClassRegister
    : public GenericRegister<std::string, StrToWeightImplBaseT,
                             WeightClassRegister> {
 protected:
  std::string ConvertKeyToSoFilename(absl::string_view key) const final {
...
#endif  // OPENFST_SCRIPT_WEIGHT_CLASS_H_
