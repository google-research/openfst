...
absl_nullable std::unique_ptr<EncodeMapperClass> ReadEncodeMapper(
...
absl_nullable std::unique_ptr<EncodeMapperImplBase> CreateEncodeMapper(
    absl::string_view arc_type, uint8_t flags, EncodeType type) {
  static const auto* reg =
      EncodeMapperClassIORegistration::Register::GetRegister();
...
EncodeMapperClass::EncodeMapperClass(absl::string_view arc_type, uint8_t flags,
                                     EncodeType type)
    : impl_(CreateEncodeMapper(arc_type, flags, type)) {}

absl_nullable std::unique_ptr<EncodeMapperClass> EncodeMapperClass::Read(
